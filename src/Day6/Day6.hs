...
import           Utils               (freqs, maxPairBy, minimumByExcl)
...
solve = fmap (solve1 &&& solve2) . parse
  where solve1 points = let bounds = getBounds points
                            area = getArea bounds
                            minMap = makeMinDistMap area points
                            infs = infinites bounds minMap
                            in show . snd . maxPairBy id . freqs
                                    . filter (`notElem` infs) . catMaybes . M.elems $ minMap
        solve2 points = let area = getArea (getBounds points)
